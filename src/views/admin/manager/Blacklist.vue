...
import { addBlacklist, delBlacklist, delBlacklistList, updateBlacklist, blacklist } from "@/network/api/blacklist"
...
const delContent = async (value) => {
    try {
        const result = await delBlacklist(value._id)
    } catch (e) {
        return
    }
...
const delMultiContent = async () => {
    ...
    try {
        const result = await delBlacklistList({ ids })
    } catch (e) {
        return
    }
    ElMessage.success(t("str.tips.delSuccess"))
...
const loadBlacklist = async () => {
    ...
    try {
        const result = await blacklist(params)
...
const submitSave = async () => {
    ...
    try {
        if (state.model.id) {
            result = await updateBlacklist(state.model.id, state.model)
        } else {
            result = await addBlacklist(state.model)
        }
    } catch (e) {
        state.loading = false
        return
