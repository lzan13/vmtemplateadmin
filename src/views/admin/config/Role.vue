<template>
    <div class="role">
        ...
        <el-table class="mt16" :data="state.dataList" :border="true" stripe empty-text="暂无数据"
            ...
            <el-table-column label="身份级别" min-width="96">
                <template #default="scope">
                    <el-tag>{{ scope.row.identity }}</el-tag>
                </template>
            </el-table-column>
        ...
        <el-dialog v-model="state.isShowEditDialog" :title="state.editTitle" :before-close="resetSave" width="30%">
            <el-form class="mr32" :model="state.model" :rules="state.rules" ref="formRef" label-width="96px"
                ...
                <el-form-item label="身份级别">
                    <el-input-number v-model="state.model.identity" :min="0" :max="999" controls-position="right"
...
import { addRole, delRole, delRoleList, updateRole, role } from "@/network/api/role"
...
const delContent = async (value) => {
    try {
        const result = await delRole(value._id)
    } catch (e) {
        return
    }
...
const delMultiContent = async () => {
    ...
    try {
        const result = await delRoleList({ ids })
    } catch (e) {
        return
    }
    ElMessage.success(t("str.tips.delSuccess"))
...
const loadRole = async () => {
    ...
    try {
        const result = await role(params)
        state.dataList = result.data.data
        state.total = result.data.totalCount
    } catch (e) { }

    // 保存角色信息到 vuex
    let roles = state.dataList.map((item: any) => {
        return {
            id: item._id,
            value: item.title,
        }
    })
    store.dispatch("infos/setRoles", roles)
...
const submitSave = async () => {
    ...
    try {
        if (state.model.id) {
            result = await updateRole(state.model.id, state.model)
        } else {
            result = await addRole(state.model)
        }
    } catch (e) {
        state.loading = false
        return
