...
import { addProfession, delProfession, delProfessionList, updateProfession, profession } from "@/network/api/profession"
...
const delContent = async (value) => {
    try {
        const result = await delProfession(value._id)
    } catch (e) {
        return
    }
...
const delMultiContent = async () => {
    ...
    try {
        const result = await delProfessionList({ ids })
    } catch (e) {
        return
    }
    ElMessage.success(t("str.tips.delSuccess"))
...
const loadProfession = async () => {
    ...
    try {
        const result = await profession(params)
        state.dataList = result.data.data
        state.total = result.data.totalCount
    } catch (e) { }

    // 保存职业信息到 vuex
    let professions = state.dataList.map((item: any) => {
        return {
            id: item._id,
            value: item.title,
        }
    })
    store.dispatch("infos/setProfessions", professions)
...
const submitSave = async () => {
    ...
    try {
        if (state.model.id) {
            result = await updateProfession(state.model.id, state.model)
        } else {
            result = await addProfession(state.model)
        }
    } catch (e) {
        state.loading = false
        return
